...
export default function CardQuestion12() {
  ...
  const submit_card = async event => {
    ...
    const c_answer_12 = {
      _id: {worker}.worker,
      card_answer_12: event.target.timeline.value
    };

    const res = await fetch('/api/question_stuff', {
      method: 'post',
      body: JSON.stringify(c_answer_12)
    })
    
    router.push('/questions/card/13')
  }
