...
export default function CardQuestion14() {
  ...
  const submit_card = async event => {
    ...
    const c_answer_14 = {
      _id: {worker}.worker,
      card_answer_14: event.target.timeline.value
    };

    const res = await fetch('/api/question_stuff', {
      method: 'post',
      body: JSON.stringify(c_answer_14)
    })
    
    router.push('/questions/card/15')
  }
