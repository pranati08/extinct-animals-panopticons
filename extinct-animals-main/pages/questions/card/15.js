...
export default function CardQuestion15() {
  ...
  const submit_card = async event => {
    ...
    const c_answer_15 = {
      _id: {worker}.worker,
      card_answer_15: event.target.timeline.value
    };

    const res = await fetch('/api/question_stuff', {
      method: 'post',
      body: JSON.stringify(c_answer_15)
    })
    
    router.push('/questions/card/16')
  }
