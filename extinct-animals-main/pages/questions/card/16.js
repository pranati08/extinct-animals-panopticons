...
export default function CardQuestion16() {
  ...
  const submit_card = async event => {
    ...
    const c_answer_16 = {
      _id: {worker}.worker,
      card_answer_16: event.target.timeline.value
    };

    const res = await fetch('/api/question_stuff', {
      method: 'post',
      body: JSON.stringify(c_answer_16)
    })
    
    router.push('/questions/card/17')
  }
