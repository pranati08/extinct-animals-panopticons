...
export default function CardQuestion7() {
  ...
  const submit_card = async event => {
    ...
    const c_answer_7 = {
      _id: {worker}.worker,
      card_answer_7: event.target.timeline.value
    };

    const res = await fetch('/api/question_stuff', {
      method: 'post',
      body: JSON.stringify(c_answer_7)
    })
    
    router.push('/questions/card/8')
  }
