...
export default function CardQuestion8() {
  ...
  const submit_card = async event => {
    ...
    const c_answer_8 = {
      _id: {worker}.worker,
      card_answer_8: event.target.timeline.value
    };

    const res = await fetch('/api/question_stuff', {
      method: 'post',
      body: JSON.stringify(c_answer_8)
    })
    
    router.push('/questions/card/9')
  }
